...
/**
 * Every status code this app can put in front of a reader.
 *
 * The enum families grow with their enums; the literal codes are written out from the places that record them, so
 * a new literal code belongs in this list as well. The list is the inventory of 16 September 2026.
 *
 * Two tests read it, and they must read the same one: `MessageTextTest` requires a sentence for every code
 * (defect 4), and `JobActionsTest` requires a recommended action for every code a job can stop with. A second
 * copy would let one of them fall behind the other without either failing.
 */
internal object ShownCodes {
